...
export function fromAIResult(aiResult) {
  return {
    ...
    sviesa: {
      ...
      ...(aiResult.ppfd?.min != null && aiResult.ppfd?.max != null
        ? { ppfd: { min: aiResult.ppfd.min, max: aiResult.ppfd.max } }
        : {}),
    },
    tresimas: aiResult.fertilizing?.intervalVasara != null ? {
    ...
    laistymasIntervalas: aiResult.watering?.intervalVasara != null ? {
      vasara:  aiResult.watering.intervalVasara,
      ziema:   aiResult.watering.intervalZiema ?? null,
      metodas: aiResult.watering.metodas ?? '',
    } : undefined,
    dormancyInfo: aiResult.dormancy?.reikia != null ? {
      reikia: aiResult.dormancy.reikia,
      tipas:  aiResult.dormancy.tipas ?? null,
    } : undefined,
    vanduo: { lygis: 'vidutiniškai', taskai: 2 },
    augimo_greitis: '',
    sunkumas: 2,
    toksiskas: false,
    toksiskumo_info: '',
    aprasymas: aiResult.aiDescription ?? '',
    kilme: aiResult.origin ?? '',
    prieziura: {
