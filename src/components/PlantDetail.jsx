      ...
      {STATUS_OPTIONS.map(opt => (
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-colors ${
            current === opt.key ? `${opt.bg} ${opt.text}` : 'bg-surface-2 text-gray-400'
          }`}
        >
          <span className={`w-2 h-2 rounded-full ${current === opt.key ? opt.dot : 'bg-gray-300'}`} />
          {opt.label}
        </button>
      ))}
    </div>
...
function WateringCard({ plant, section }) {
  ...
  return (
    <div className="bg-sky-50 border border-sky-100 rounded-2xl px-4 py-3 flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        {wc.metodas && (
          <p className="text-[11px] text-sky-500 italic mt-1">{wc.metodas}</p>
        )}
      </div>
    </div>
  )
...
function FertilizingCard({ plant, section }) {
  if (section !== 'auginama') return null
      ...
      <div className="bg-orange-50 border border-orange-200 rounded-2xl px-4 py-3 flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-orange-700">
            Tręšimas vėluoja {days} d{days === 1 ? '.' : '.'}!
          </p>
...
function DormancyCard({ plant, section }) {
  if (section !== 'auginama') return null
    ...
    <div className={`border rounded-2xl px-4 py-3 flex gap-3 ${c.bg}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {df.ziemojimas && df.window !== 'waking' && (
          <p className={`text-[10px] mt-1.5 italic ${c.subColor}`}>
            {df.ziemojimas}
          </p>
        )}
      </div>
                ...
                {section !== 'istorija' && (
                  <div className="relative flex-shrink-0">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-semibold bg-black/30 text-white active:bg-black/50 transition-colors"
                    >
                      <span className={`w-2 h-2 rounded-full ${statusDot(status).dot}`} />
                    ...
                    {showStatusMenu && (
                      <>
                        ...
                        <div className="absolute right-0 top-full mt-1.5 bg-white rounded-2xl shadow-xl border border-warm-border p-1.5 z-50 min-w-[140px]">
                          {STATUS_OPTIONS.filter(opt => section === 'auginama' || opt.key !== 'numire').map(opt => (
                            <button
                              key={opt.key}
                              onClick={() => {
  setStatusMenu(false)
  if (opt.key === 'numire') { onAction?.('died', plant); onClose?.() }
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  {section !== 'istorija' && (
                    <div className="relative flex-shrink-0">
                      <button
                        onClick={() => setStatusMenu(v => !v)}
                        className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl text-xs font-semibold mt-0.5 transition-colors ${statusDot(status).bg} ${statusDot(status).text}`}
                      >
                        <span className={`w-2 h-2 rounded-full ${statusDot(status).dot}`} />
                        {statusDot(status).label}
                        <ChevronDown size={10} className="opacity-60" />
